...
static void ILI9341_GPIO_Config(void)
{
	...
	/* 配置FSMC相对应的控制线
	 * FSMC_NOE   :LCD-RD	读使能 : 读信号
	 * FSMC_NWE   :LCD-WR	写使能 : 写信号
	 * FSMC_NE1   :LCD-CS	片选   : 片选
	 * FSMC_A16   :LCD-DC	地址/数据 : 数据/命令信号
	 */
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
...
void LCD_Init(void)
{
	...
	/********固定显示 这三行是不变的*******/
	ILI9341_DispStringLine_EN(LINE(2), " 3.2 inch LCD is working");
	ILI9341_DispStringLine_EN(LINE(1), "Image resolution:240x320 px");
	ILI9341_DispStringLine_EN(LINE(0), "ILI9341 LCD driver");
}
